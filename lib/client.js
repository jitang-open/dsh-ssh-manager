...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const TAG_ID = 'dsh-ssh-manager/panel.css';
			...
			function renderEditor() {
				return h('div', null,
					...
					h('div', { className: 'dsh-ssh-form' },
						field('名称', input('name', '腾讯云服务器')),
						field('系统类型', h('select', {
							className: 'dsh-ssh-input',
							value: draft.kind,
							onChange: (event) => setDraft({ ...draft, kind: event.target.value })
						}, h('option', { value: 'linux' }, 'Linux / macOS'), h('option', { value: 'windows' }, 'Windows'))),
						field('主机地址', input('host', '119.45.179.55')),
						field('端口', input('port', '22')),
						field('用户名', input('user', 'ubuntu')),
						field('私钥路径（留空则用默认密钥/agent）', input('keyPath', '/Users/mac/.ssh/id_ed25519'))
					),
					field('备注', input('note', '用途说明')),
					field('标签（逗号分隔）', input('tags', '腾讯云, 公网')),
